...
repo; ad-hoc jsdom scripts have been used for smoke testing but are not
committed.
...
`NavRail` (desktop) must be rendered as a direct flex sibling of `<main>`;
`NavBar` (mobile) must be rendered full-bleed *outside* that flex row. They were
originally one component returning a fragment, which made the mobile bar a flex
child and rendered it as a narrow column beside the content. Do not recombine
them.
...
`Abdulaziz_Alsuhaibani_FullStackDeveloper.pdf` at the repo root is the original
CV, also copied to `public/` so the site can serve it. Its text is not
extractable by naive tools — it uses a custom subset font encoding, so text must
be pulled by inflating the FlateDecode streams and applying the ToUnicode CMap.
`src/data/content.ts` already holds the fully extracted result; use that rather
than re-parsing the PDF.

`my-portifolio-ideation.md` records the original brief.
