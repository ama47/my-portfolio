/**
 * Inline icons. Kept in-repo rather than pulling an icon package, in line with
 * the react + react-dom only dependency rule.
 *
 * Every icon is decorative — each one is rendered next to a text label — so
 * `aria-hidden` is baked in rather than left to the call site.
 *
 * Sources of the path data:
 * - GitHub, W3Schools — Simple Icons (CC0 1.0)
 * - LinkedIn (`linkedin-plain`), AWS (`amazonwebservices-original-wordmark`) —
 *   Devicon (MIT). Simple Icons no longer ships either mark.
 * The brand marks carry `fill="currentColor"` so they take the surrounding
 * colour token and work in both themes; the original brand colours are dropped
 * on purpose.
 */
...
/** Shared attributes for the hand-drawn glyphs, matching CertCard's fallback. */
const strokeProps = {
  viewBox: '0 0 24 24',
  fill: 'none',
  stroke: 'currentColor',
...
export function PhoneIcon({ className = 'h-4 w-4' }: IconProps) {
  return (
    <svg aria-hidden="true" className={className} {...strokeProps}>
      <path d="M7.5 3.5h-2a2 2 0 0 0-2 2.13C4.1 13.9 10.1 19.9 18.37 20.5a2 2 0 0 0 2.13-2v-2a1.5 1.5 0 0 0-1.2-1.47l-2.6-.52a1.5 1.5 0 0 0-1.47.6l-.9 1.2a13.6 13.6 0 0 1-5.64-5.64l1.2-.9a1.5 1.5 0 0 0 .6-1.47l-.52-2.6A1.5 1.5 0 0 0 7.5 3.5Z" />
    </svg>
  );
}

/** Award ribbon — the fallback for a certification with no issuer logo. */
...
export function LinkedinIcon({ className = 'h-4 w-4' }: IconProps) {
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 128 128" fill="currentColor">
...
/**
 * The AWS wordmark. Devicon draws it inside a 128 square with a lot of empty
 * space above and below, so the viewBox is cropped to the artwork — otherwise
 * the lettering shrinks to nothing at plate size.
 */
export function AwsLogo({ className = 'h-6 w-auto' }: IconProps) {
  return (
    <svg aria-hidden="true" className={className} viewBox="0 26 128 76" fill="currentColor">
...
export function W3SchoolsLogo({ className = 'h-6 w-6' }: IconProps) {
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 24 24" fill="currentColor">
